...
const StepFasilitasTipeKost = () => {
  ...
  const { saveFacilities, refetchKostTypeOwner } = useKostType({
    ...
    setOnNext(
      handleSubmit((data) => {
        ...
        saveFacilities(
          ...
          {
            onSuccess: async (res) => {
              await refetchKostTypeOwner();
              setProgressStep(3);
              setCurrentStep(3);
              router.replace(
                `/dashboard/kost-type/create?kost_id=${res.data.kostId}&kost_type_id=${res.data.kostTypeId}&step=3`
              );
            },
          }
        );
